...
namespace AsmJit.Common
{
    public static unsafe class UnsafeMemory
    {
        ...
        internal static Pointer Allocate(int count, bool @protected = false)
        {
            ...
            if (mode.IsSet(ProtectedMemoryMode.Executable)) protectFlags |= mode.IsSet(ProtectedMemoryMode.Writable) ? Win32.MemoryProtectionType.ExecuteReadWrite : Win32.MemoryProtectionType.ExecuteRead;
            else protectFlags |= mode.IsSet(ProtectedMemoryMode.Writable) ? Win32.MemoryProtectionType.ReadWrite : Win32.MemoryProtectionType.ReadOnly;
            ...
            if (@base.IsInvalid) return Pointer.Invalid;
        ...
        internal static Pointer Allocate(int count, int alignment)
        {
            ...
            byte* tmp;
            if ((tmp = (byte*)Marshal.AllocHGlobal(count + alignment)) == (void*)0) return Pointer.Invalid;
            var ptr = (byte*)((uint)(tmp + alignment - 1) & (~(uint)(alignment - 1)));

            if (ptr == tmp) ptr += alignment;
            *(ptr - 1) = (byte)(ptr - tmp);
        ...
        {
            if (p.Flags.IsSet(PointerFlags.Protected)) throw new ArgumentException("Protected memory cannot be realocated");
            var tmp = Allocate(size);
            if (p != Pointer.Invalid) Copy(tmp, p, p.DataSize > size ? size : p.DataSize);
            Free(p);
            return tmp;
        }
        ...
        internal static void Free(Pointer ptr)
        {
            ...
            else Win32.NativeMethods.VirtualFreeEx(_processHandle, ptr, (UIntPtr)0, Win32.VirtualFreeType.Release);
        }
        ...
        internal static int Compare(Pointer p1, Pointer p2, int count) => memcmp(p1, p2, (UIntPtr)count);

        internal static void Fill(Pointer ptr, byte value, int count) => _memsetDelegate(ptr, value, count);
