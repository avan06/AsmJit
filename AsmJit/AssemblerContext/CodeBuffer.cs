...
namespace AsmJit.AssemblerContext
{
    internal sealed class CodeBuffer
    {
        ...
        public CodeBuffer(Assembler assembler) => _assembler = assembler;

        private int Capacity => (int)(_end - _buffer);

        private int Offset => (int)(_cursor - _buffer);

        private int TrampolinesSize => _trampolinesSize;

        public int CodeSize => Offset + _trampolinesSize;

        private int RemainingSpace => (int)(_end - _cursor);
        ...
        private int RelocateCode(ref Pointer dst) => RelocateCode(ref dst, Pointer.Invalid);

        private int RelocateCode(ref Pointer dst, Pointer baseAddress)
        {
            if (baseAddress == Pointer.Invalid) baseAddress = dst;
                ...
                switch (encoding)
                {
                    ...
                    case InstructionEncoding.X86RegRm:
                        ...
                        if (OperandsAre(OperandType.Register, OperandType.Register))
                        {
                            ...
                            {
                                IllegalInstruction(string.Format("Operands are Register & Register, but size is 1 or both are different sizes(op0 size:{0}, op1 size:{1})", op0.Size, op1.Size));
                                @continue = false;
                                break;
                            }
                        ...
                        if (OperandsAre(OperandType.Register, OperandType.Memory))
                        {
                            if (op0.Size == 1)
                            {
                                IllegalInstruction("Operands are Register & Memory, but op0 size is 1)");
                                @continue = false;
                                break;
                            }
                    ...
                    case InstructionEncoding.X86Arith:
                        ...
                        if (OperandsAre(OperandType.Memory, OperandType.Immediate))
                        {
                            ...
                            if (memSize == 0)
                            {
                                IllegalInstruction("Operands are Memory & Immediate, but op0 size is 0");
                                @continue = false;
                                break;
                            }
                    ...
                    case InstructionEncoding.X86BSwap:
                        if (OperandsAre(OperandType.Register))
                        {
                            if (op0.Size < 4)
                            {
                                IllegalInstruction(string.Format("Encoding is X86BSwap and Operands is Register, but op0 size is less than 4", op0.Size));
                                @continue = false;
                                break;
                            }
                    ...
                    case InstructionEncoding.X86Jecxz:
                        if (OperandsAre(OperandType.Register, OperandType.Label))
                        {
                            ...
                            if (Constants.X64)
                            {
                                if (op0.Size == 4) EmitByte(0x67);
                            }
                            else
                            {
                                if (op0.Size == 2) EmitByte(0x67);
                            }
        ...
        private void EmitX86M()
        {
            ...
            if (_eh.ModRmMemory.HasBaseOrIndex() && _eh.ModRmMemory.MemoryType != MemoryType.Label)
            {
                if (Constants.X64)
                {
                    if (_eh.ModRmMemory.HasGdpBase) EmitByte(0x67);
                }
                else
                {
                    if (!_eh.ModRmMemory.HasGdpBase) EmitByte(0x67);
        ...
        private void EmitAvxV()
        {
            ...
            else
            {
                ...
                if (_eh.ModRmMemory.MemoryType == MemoryType.Label)
                {
                    if (Constants.X64)
                    {
                        IllegalAddressing("MemoryType is Label, cannot X64");
                        return;
                    }
                ...
                if (Constants.X64)
                {
                    switch (_eh.ModRmMemory.MemoryType)
                    {
                        ...
                        case MemoryType.Label:
                            ...
                            if (_eh.Index != -1)
                            {
                                IllegalDisplacement(string.Format("MemoryType is Label and Index:{0} is invalid", _eh.Index));
                                return;
                            }
                        ...
                        default:
                            ...
                            if (_eh.Index != -1)
                            {
                                IllegalDisplacement(string.Format("MemoryType is {0} and Index:{1} is invalid", _eh.ModRmMemory.MemoryType, _eh.Index));
                                return;
                            }
        ...
        private void IllegalAddressing(string message = null) => throw new InvalidDataException(message);

        private void IllegalDisplacement(string message = null) => throw new InvalidDataException(message);

        private void IllegalInstruction(string message = null) => throw new ArgumentException(message);
